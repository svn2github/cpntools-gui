...
fun gen_occfun (trstr,(pgstr,withcode:bool, withtime,outtimed))
    ...
    ("fun CPN'OGO"^trstr^"(CPN'inst,CPN'BTT"^
     ...
     "(CPN'transition"^(CPN'OGBTconvGen.findmlnoforbinding trstr)^
     ".CPN'occfun(CPN'inst,CPN'b);\n"^
     "CPN'BTT"^(CPN'OGBTconvGen.findmlnoforbinding trstr)^" CPN'b) ")::
     (!code_storage);
             

 fun gen_transfun()
    = (code_temp:="local\n"^
                  "fun CalcTimeEnab CPN'ctime=(";
       code_temp:= !code_temp^"())in\n"^               
	          "fun CPN'OGCalcEnab()= (\n";
       firsttri := true;
			 ...
			 then
				 ...
				 (fn {instmlno=instmlno,instno=instno,...}
					 ...
					 code_temp:= !code_temp^(if !firsttri then (firsttri := false;"") else ";\n")^
						     ("CPN'OGE"^trmlno^"("^instno^","^internalid^")")
		                     end(*let*))
				 instlist
			 else
			     []
	...
	(if (CPN'Time.name <> "unit") then
	     (code_temp := !code_temp^"; \n (if CPN'OGEnabData.get_cands() = [] then \n\
				      \ (if (CPN'Sim.reset_scheduler();\n";
	      if (CPN'Time.name <> "unit") then
		  (map 
		       (fn {trlist = trlist, pgno = pgno, instlist = instlist,...}
			   => map
				  (fn {trmlno = trmlno, trname=trname,...}
				      => if CPN'TransitionTable.is_transition trmlno then
						 ...
						 map
						     (fn {instmlno=instmlno,instno=instno,...} =>
							 code_temp:= !code_temp^
								     ("CPN'Sim.check_enab(\""^trmlno^"\","^instno^");\n"))
						     instlist
					     else
						 []
					 else [])
	      ...
	      code_temp := !code_temp^"CPN'Sim.increase_model_time()) then (CPN'Sim.reset_scheduler();\n";
	      (map 
		       (fn {trlist = trlist, pgno = pgno, instlist = instlist,...}
			   => map
				  (fn {trmlno = trmlno, trname=trname,...}
				  ...
				  trlist)
		       (!CPN'NetCapture.Net));
	      code_temp := !code_temp^"CPN'OGCalcEnab()) else ())\n\
				      \ else ());")
       ...
       code_temp:= !code_temp^
		       ...
		       (* FIXME: Get rid of this function - doesn't do anything  *)
                    "fun CPN'OGCalcEnabTimeBinds()=\n("^
                    (if (CPN'Time.name <> "unit")
                     then "" (*"Time.model_time := !CPN'OGTimeEnabData.curr_min;\n"*)
                     else "")^
                     "CalcTimeEnab false);\n"^
                    "end;\n";
