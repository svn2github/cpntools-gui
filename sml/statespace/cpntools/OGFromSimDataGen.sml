...
fun gen_field {fieldname=fieldname, colset=colset} =
    code_temp := (!code_temp)^
		 let
		     val [plmlno,instno] = String.tokens (fn CPN'x => CPN'x = (#"P") orelse CPN'x = (#"F") orelse CPN'x = (#"'")) fieldname
		 in
		     ((if !first
		       then (first := false; "")
		       else ",\n")^
		      fieldname^"=CPN'"^colset^"'mss.Insert(CPN'MS.sort_ms "^(CPN'CSTable.get_prime_cs colset)^".lt ("^
		      (if CPN'CSTable.is_timed colset then "map Time.col " else "")^
		      "(CPN'place"^plmlno^".get "^instno^")))"^
		      (if CPN'CSTable.is_timed colset
		       then ",\n"^fieldname^"t="^
			    ...
			    "((CPN'place"^plmlno^".get "^instno^"))) in \n"^
...
fun gen_localstore ("pgglobfus",ref fieldlist)
  ...
  | gen_localstore (recname,ref fieldlist)
   = (code_temp:=("fun CPN'OGstore_"^recname^"()\n"^
          "= let\n"^
              "val "^recname^"rec\n"^
                 "=CPN'OGLocal"^recname^"\n{");
