    ...
    = let
       ...
       fun processBindElem CPN'time (CPN'tri,CPN'b) (* execute CPN'tri with CPN'b and store *)
		 ...
		 (* execute CPN'ttri with CPN'b, and record the new binding - this is to handle output form codeseg in the right way.... *)
  	      ...
  	      val CPN'b = (CPN'OGExecute(CPN'tri,CPN'b) 
			   handle CPN'e => 
			       (let
				    val errstr = "Exception "^exnName(CPN'e)^" raised while executing "^(#1 CPN'tri)^" in some binding in Node #"^Int.toString(CPN'n)^"."
      ...
      in
	   ...
	   CPN'OGEnabData.init();  (* initialization for immediate trans*)
           CPN'OGTimeEnabData.init();  (* initialization for timed trans*)
	    ...
	    handle CPN'e => (let
           ...
           if (CPN'Time.name <> "unit") andalso
              (CPN'OGEnabData.get_cands()<>[])andalso
		  CPN'Time.lt((!CPN'Time.model_time),CPN'Time.maketime(CPN'Time.toString (!CPN'OGTimeEnabData.curr_min)))
           then (* only immediate trans should be executed at current model time *)
             CPN'OGTimeEnabData.init()
           else
               CPN'OGCalcEnabTimeBinds();
           
           if (!CPN'OGEnabData.bindlimit_reached)orelse
             (CPN'OGTimeEnabData.any_limit()) 
           then NewCalcStat:=PartProc
           else ();
           
	   (* generate succesors for arcs stored as timed and untimed enabled binding elements *)
	   map (processBindElem (!CPN'Time.model_time)) (remdupl ((CPN'OGEnabData.get_cands())^^(CPN'OGTimeEnabData.get_cands())));
           ...
           CPN'OGNodeSel.AppendList(CPN'OGListUtils.mapfilter( (*add the new nodes to the selection list*)
                    ref(fn CPN'outn
