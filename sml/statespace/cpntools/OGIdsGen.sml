...
local
	...
	handle CPN'AvlTree.ExcAvlInsert => (* recname was there => add fieldname to the list *)
		   ...
		   val fieldsref = (CPN'AvlTree.AvlLookup(DataRecFields,recname))
               in
		   if CPN'OGListUtils.mem (!fieldsref) {fieldname=fieldname,colset=colset}
	    ...
	    CPN'AvlTree.AvlInsert PlaceInsts
				  ((plmlno^" "^instno),
				   ...
				   then (*non-fused, non-port place*)
				       (store_datarecfield("pg"^pgmlno,"P"^plmlno^"'"^instno,colset);
					
					{simplace="CPN'place"^plmlno,
					 siminst=instno,
					 OGdatarec="pg"^pgmlno^"'"^instno,
					 OGdatarecfield="P"^plmlno^"'"^instno,
					 pli=fullid,
					 plitype=nonport,
					 colset=colset})
				       
				   else (*page or global fus*)
				       (store_datarecfield("pgglobfus","F"^valOf(fusgrid)^"'"^instno,colset);
					
					{simplace="CPN'place"^(valOf fusgrid),
					 siminst=instno,
					 OGdatarec="pgglobfus",
					 OGdatarecfield="F"^(valOf fusgrid)^"'"^instno,
					 pli=fullid,
					 plitype=nonport,
					 colset=colset}
					)
