...
local
 ...
 fun StandardReport()
    = let
      ...
      in
       DSUI_SetStatusBarMessage("OG Generation:"^
               "#Nodes ="^nodecountstr^"/"^
                                totalnodecountstr^" "^
               "#Arcs  ="^arccountstr^"/"^
                                totalarccountstr^" "^
...
fun NoOfSecs () = Int.fromLarge(CPN'OGStopCrit.GetTotalTimeSecs ());
...
CPN'OGInspection.SetParams{
     Frequency= 60,
     Action= CPN'OGInspection.StandardReport};
