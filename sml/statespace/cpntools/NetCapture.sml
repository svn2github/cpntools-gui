...
local
    ...
    fun BuildInsts (id,0) = nil
      | BuildInsts (id,num_of_insts) =
	{instmlno = id, instno = Int.toString num_of_insts}::BuildInsts(id,num_of_insts - 1)
	...
	let
	    ...
	    fun BuildPages' nil = nil
	      | BuildPages' ((id,{page = {name,places,transitions,included,prime},
				  decl,super_pages,sub_pages})::CPN'xs) =
		{pllist = BuildPlaces places,
		 trlist = BuildTransitions transitions,
		 instlist = BuildInsts (id,CPN'InstTable.get_no_of_inst id),
