         ...
         let 
          fun insert (CPN'root as (ref emptymssnode),CPN'k)
             = (CPN'root := mssnode {key=CPN'k, value=[ref CPN'ms1], right = ref emptymssnode, 
                ...
                raise taller)
            | insert (CPN'root as (ref (mssnode {key, left, right, bf, value=msreflist})),CPN'k)
             = if (CPN'k = key) 
               then
                if isnotin(msreflist,CPN'ms1)
                then 
                 (CPN'root := mssnode {key=key, value=(ref CPN'ms1)::msreflist,
                ...
                if (CPN'k < key)
                then (insert(left,CPN'k) handle taller =>
                       (case !bf of
                         EH => (bf := LH; raise taller) |
                         LH => (leftbalance CPN'root;()) |
                         RH => bf := EH))
                else (insert(right,CPN'k) handle taller =>
                       (case !bf of 
   ...
   fun App func =
    let 
     fun app (ref emptymssnode)= ()
       | app (ref (mssnode {left, right, value,...}))
